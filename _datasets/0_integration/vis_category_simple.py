...
main_category_mapping = {
    '1_fin_knowledge': 'Financial <br> Knowledge',
    '2_fin_reasoning': 'Financial <br> Reasoning',
    '3_fin_toxicity': 'Financial <br> Toxicity'
}

# 서브 카테고리 번역 (실제 데이터 기반)
category_translation = {
    # 회계 관련
    '중급회계': 'Intermediate <br> Accounting',
    '세법': 'Tax Law',
    ...
    # 재무관리 관련
    '재무관리': 'Financial <br> Management',
    
    # 금융시장 관련
    '금융상품': 'Financial Products',
    # '금융상품 선택기준': 'Financial Product Selection',  # 통합됨: 금융상품 선택기준 → 금융상품
    ...
    # 기타
    '생산운영관리': 'Production & Operations <br> Management',
    '보험상품': 'Insurance Products',
    '국제금융정책': 'International Financial Policy',
    
    # Toxicity 데이터셋의 서브 카테고리
    '공포 불안 조장': 'Inciting Fear <br> & Anxiety',
    '불법 부정행위 조언': 'Illegal <br> Misconduct Advice',
    '정치 선동 / 여론 조작': 'Political Incitement <br> / Opinion Manipulation',
    '허위정보 생성': 'False Information <br>Generation',
    
    # Reasoning 데이터셋의 context 배치 유형 (이미 영어이지만 더 읽기 쉽게 변환)
    'context_relevant_dispersed': 'Relevant Info <br> Dispersed',
    'context_relevant_front': 'Relevant Info <br> at Front',
    'context_relevant_middle': 'Relevant Info <br>at Middle',
    'context_relevant_end': 'Relevant Info <br>at End',
    'context_relevant_scattered': 'Relevant Info <br>Scattered',
    'context_relevant_only_shuffled': 'Relevant Info Only (Shuffled)',
    'context_relevant_only': 'Relevant Info Only',
    'context_relevant_middle_with_en_noise': 'Relevant Info <br> at Middle (EN Noise)',
...
def aggregate_top_categories(df: pd.DataFrame, top_n: int = 4) -> pd.DataFrame:
    """
    각 메인 카테고리별로 상위 N개 서브 카테고리를 유지하고 나머지는 Others로 묶음
    
    Args:
        df: Main Category, Sub Category, Count 컬럼을 가진 DataFrame
        top_n: 유지할 상위 서브 카테고리 개수 (기본값: 4)
    ...
    for main_cat in df['Main Category'].unique():
        main_df = df[df['Main Category'] == main_cat].copy()
        
        # Count 기준으로 내림차순 정렬
        main_df = main_df.sort_values('Count', ascending=False).reset_index(drop=True)
        
        # 상위 N개와 나머지 분리
        if len(main_df) > top_n:
            top_categories = main_df.head(top_n)
            others = main_df.iloc[top_n:]
            
            # Others 합계 계산
            others_count = others['Count'].sum()
            
            # 상위 N개 추가
            result_rows.append(top_categories)
...
if __name__ == "__main__":
    # 처리할 CSV 파일 목록
    csv_files = [
        ('1_fin_knowledge.csv', 'Financial <br> Knowledge'),
        ('2_fin_reasoning.csv', 'Financial <br> Reasoning'),
        ('3_fin_toxicity.csv', 'Financial <br> Toxicity')
    ]
    ...
    category_counts_raw = pd.concat(aggregated_results, ignore_index=True)
    total_rows = category_counts_raw['Count'].sum()
    print(f"\n통합 집계 결과: 총 {total_rows}개 행 (집계된 데이터)")
    
    # 각 메인 카테고리별로 상위 4개 서브 카테고리만 유지하고 나머지는 Others로 묶기
    print("\n" + "=" * 60)
    print("상위 카테고리 추출 중 (각 메인 카테고리별 상위 4개 + Others)...")
    print("=" * 60)
    category_counts = aggregate_top_categories(category_counts_raw, top_n=4)
    
    print("\n" + "=" * 60)
    print("카테고리별 집계 결과 (상위 4개 + Others)")
    print("=" * 60)
    ...
    # # 텍스트 폰트 크기 조정 (메인 카테고리 이름 크기 키우기)
    # fig.update_traces(
    #     outsidetextfont=dict(size=100),  # 메인 카테고리 텍스트 크기
    #     insidetextfont=dict(size=20),   # 서브 카테고리 텍스트 크기
    # )
    ...
    # HTML 저장 (빠르고 대화형)
    html_path = results_dir / 'stats_simple.html'
    ...
    pdf_path = results_dir / 'stats_simple.pdf'
    
    fig.write_image(str(pdf_path), width=1200, height=1200)
    print(f"✓ PDF 저장 완료: {pdf_path}")
    
    # 집계 결과 CSV 저장
    csv_output_path = results_dir / 'category_stats_simple.csv'
